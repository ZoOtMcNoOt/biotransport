...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda: bt.reference.SinusoidalBC(frequency=0.0),
            "frequency must be positive",
        ),
        (lambda: bt.reference.RampBC(duration=0.0), "duration must be positive"),
        (lambda: bt.reference.SquareWaveBC(duty_cycle=1.1), "duty_cycle"),
        (
            lambda: bt.reference.ArterialPressureBC(systolic=80.0, diastolic=120.0),
            "systolic must be greater",
        ),
        (
            lambda: bt.reference.RespiratoryBC(inspiration_fraction=1.0),
            "inspiration_fraction",
        ),
        (lambda: bt.reference.DrugInfusionBC(bolus_duration=-1.0), "bolus_duration"),
        (lambda: bt.reference.CompositeBC(operation="subtract"), "operation"),
    ],
)
...
def test_arterial_template_has_declared_extrema_and_period():
    waveform = bt.reference.ArterialPressureBC(
        systolic=120.0,
        diastolic=80.0,
        heart_rate=60.0,
...
def test_periodic_waveforms_reduce_extreme_phases_without_nan():
    sinusoid = bt.reference.SinusoidalBC(frequency=1e308, phase=1e308)
    square = bt.reference.SquareWaveBC(frequency=1e308, phase=1e308)
    ...
    with pytest.raises(ValueError, match="period outside the finite float64 range"):
        bt.reference.SinusoidalBC(frequency=np.nextafter(0.0, 1.0)).period()


def test_waveform_derived_arithmetic_is_finite_or_fails_loudly():
    maximum = np.finfo(float).max
    ramp = bt.reference.RampBC(start_value=-maximum, end_value=maximum)
...
def test_venous_mean_pressure_is_the_declared_cycle_mean():
    waveform = bt.reference.VenousPressureBC(
...
def test_cardiac_output_mean_is_the_declared_cycle_mean():
    waveform = bt.reference.CardiacOutputBC(
        mean_flow=5.0,
        peak_flow=25.0,
        heart_rate=72.0,
...
def test_composite_reports_only_an_established_common_period():
    periodic = bt.reference.CompositeBC(
        [
            bt.reference.SinusoidalBC(frequency=1.0),
            bt.reference.SinusoidalBC(frequency=2.0 / 3.0),
        ]
    )
    nonperiodic = bt.reference.CompositeBC(
        [bt.reference.ConstantBC(2.0), bt.reference.RampBC()]
    )
...
def test_sampling_and_rate_conversions_validate_their_domains():
    with pytest.raises(ValueError, match="bpm must be positive"):
        bt.reference.heart_rate_to_period(0.0)
    with pytest.raises(ValueError, match="T must be positive"):
        bt.reference.period_to_heart_rate(-1.0)
    with pytest.raises(ValueError, match="t_end"):
        bt.reference.sample_waveform(bt.reference.ConstantBC(1.0), 1.0, 0.0)
    with pytest.raises(ValueError, match="num_points"):
        bt.reference.sample_waveform(bt.reference.ConstantBC(1.0), num_points=0)
    subnormal = np.nextafter(0.0, 1.0)
    with pytest.raises(ValueError, match="outside the finite float64 range"):
        bt.reference.heart_rate_to_period(subnormal)
    with pytest.raises(ValueError, match="outside the finite float64 range"):
        bt.reference.period_to_heart_rate(subnormal)
...
def test_reference_solver_preserves_mass_with_zero_neumann_boundaries():
    ...
    with pytest.warns(RuntimeWarning, match="legacy Python/NumPy reference"):
        result = bt.reference.solve_pulsatile(problem, 0.05, {}, dt=0.001)
...
def test_reference_solver_applies_dynamic_dirichlet_at_the_new_time_level():
    ...
    waveform = bt.reference.CustomBC(lambda time: 2.0 * time, T=0.0)
...
def test_reference_solver_rejects_an_unstable_step():
    ...
    with pytest.raises(ValueError, match="stability limit"):
        bt.reference.solve_pulsatile(problem, 0.1, {}, dt=0.01)
...
def test_reference_solver_rejects_physics_it_does_not_implement():
    ...
    with pytest.raises(NotImplementedError, match="reactions"):
        bt.reference.solve_pulsatile(reaction_problem, 0.1, {}, dt=0.001)

    _, advection_problem = _problem_1d()
    advection_problem.velocity(0.1)
    with pytest.raises(NotImplementedError, match="advection"):
        bt.reference.solve_pulsatile(advection_problem, 0.1, {}, dt=0.001)
    ...
    with pytest.raises(NotImplementedError, match="variable diffusivity"):
        bt.reference.solve_pulsatile(variable_problem, 0.1, {}, dt=0.001)
...
def test_reference_solver_rejects_unsupported_boundaries_and_dimensions():
    ...
    with pytest.raises(NotImplementedError, match="Robin"):
        bt.reference.solve_pulsatile(robin_problem, 0.1, {}, dt=0.001)

    with pytest.raises(NotImplementedError, match="Robin"):
        bt.reference.solve_pulsatile(
            robin_problem,
            0.1,
            {bt.Boundary.Right: bt.reference.ConstantBC(1.0)},
            dt=0.001,
        )
    ...
    with pytest.raises(ValueError, match="one-dimensional"):
        bt.reference.solve_pulsatile(problem_2d, 0.1, {}, dt=0.001)
    ...
    with pytest.raises(ValueError, match="only Boundary.Left"):
        bt.reference.solve_pulsatile(
            problem,
            0.1,
            {bt.Boundary.Top: bt.reference.ConstantBC(1.0)},
            dt=0.001,
        )
...
def test_zero_diffusivity_requires_an_explicit_temporal_resolution():
    ...
    with pytest.raises(ValueError, match="dt is required"):
        bt.reference.solve_pulsatile(
            problem, 1.0, {bt.Boundary.Left: bt.reference.ConstantBC(1.0)}
        )
    ...
    with pytest.warns(RuntimeWarning):
        result = bt.reference.solve_pulsatile(
            problem,
            1.0,
            {bt.Boundary.Left: bt.reference.ConstantBC(1.0)},
            dt=0.1,
        )
...
def test_step_planning_never_rounds_down_a_tiny_final_remainder():
    ...
    with pytest.raises(ValueError, match="more than max_steps=1"):
        bt.reference.solve_pulsatile(problem, final_time, {}, dt=stable_dt, max_steps=1)

    with pytest.warns(RuntimeWarning):
        result = bt.reference.solve_pulsatile(
            problem, final_time, {}, dt=stable_dt, max_steps=2
        )
...
def test_step_planning_uses_the_exact_float_ratio_not_rounded_products():
    ...
    with pytest.raises(ValueError, match="more than max_steps=3"):
        bt.reference.solve_pulsatile(problem, final_time, {}, dt=step_dt, max_steps=3)
...
def test_decimal_endpoint_keeps_its_exact_remainder_step():
    ...
    with pytest.warns(RuntimeWarning):
        result = bt.reference.solve_pulsatile(
            problem, 0.1, {}, dt=0.0001, max_steps=1001
        )
...
def test_subnormal_dt_fails_with_the_step_guard_instead_of_overflowing():
    ...
    with pytest.raises(ValueError, match="more than max_steps=10"):
        bt.reference.solve_pulsatile(problem, 1.0, {}, dt=subnormal_dt, max_steps=10)
...
def test_tiny_diffusivity_is_not_lost_when_the_stable_dt_overflows():
    ...
    with pytest.warns(RuntimeWarning):
        result = bt.reference.solve_pulsatile(
            problem, step_dt, {}, dt=step_dt, max_steps=1
        )
...
def test_stable_limit_avoids_large_spacing_intermediate_overflow():
    ...
    with pytest.warns(RuntimeWarning):
        automatic = bt.reference.solve_pulsatile(problem, 100.0, {})
    with pytest.warns(RuntimeWarning):
        explicit = bt.reference.solve_pulsatile(problem, 100.0, {}, dt=40.0)
...
def test_stable_limit_avoids_tiny_spacing_intermediate_underflow():
    ...
    with pytest.warns(RuntimeWarning):
        result = bt.reference.solve_pulsatile(problem, 1e-300, {})
...
def test_custom_waveform_cannot_mutate_the_transport_problem():
    ...
    waveform = bt.reference.CustomBC(mutate_problem)
    with pytest.warns(RuntimeWarning):
...
def test_solve_callback_cannot_mutate_the_transport_problem():
    ...
    with pytest.warns(RuntimeWarning):
        with pytest.raises(RuntimeError, match="TransportProblem was mutated"):
            bt.reference.solve_pulsatile(
                problem, 0.02, {}, dt=0.01, callback=mutate_problem
            )
